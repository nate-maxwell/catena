...
from catena.application.nodes.math import IMAGE_NODE_COLOR


class SinNode(CatenaNode):
    """A node that applies a sine wave, either as a generate or as a remap of an input."""
    ...
    def process(
        ...
        phase_rad = numpy.deg2rad(phase)
        wave = numpy.sin(2 * numpy.pi * frequency * gray + phase_rad)
        wave = (wave + 1.0) * 0.5
        return numpy.repeat(wave[:, :, None], 3, axis=2).astype(numpy.float32)
