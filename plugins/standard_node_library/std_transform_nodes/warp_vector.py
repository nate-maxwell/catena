...
class VectorWarpNode(api.CatenaNode):
    ...
    def process(
        ...
        if displacement is None or strength <= 0:
            return image
        ...
        offset_x = (displacement[:, :, 2] * 2.0 - 1.0) * strength
        offset_y = (displacement[:, :, 1] * 2.0 - 1.0) * strength
