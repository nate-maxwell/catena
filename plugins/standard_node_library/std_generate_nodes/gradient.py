from typing import Optional

import numpy

from catena import api
from std_generate_nodes.generator import GeneratorNode


class GradientNode(GeneratorNode):
    """A node that generates a ar gradient between two colors."""

    def __init__(self) -> None:
        super().__init__(title="Gradient")

    def _build(self) -> None:
        self.port_out = self.add_port(api.PortType.OUTPUT, "Output")

        self.add_field(
            api.FieldDefinition(
                name="color_a",
                label="Color A",
                field_type=api.FieldType.COLOR,
                default=(0, 0, 0, 255),
            )
        )
        self.add_field(
            api.FieldDefinition(
                name="color_b",
                label="Color B",
                field_type=api.FieldType.COLOR,
                default=(255, 255, 255, 255),
            )
        )
        self.add_field(
            api.FieldDefinition(
                name="angle",
                label="Angle",
                field_type=api.FieldType.FLOAT,
                default=0.0,
                min_value=-360.0,
                max_value=360.0,
            )
        )

    def process(
        self, inputs: dict[str, Optional[numpy.ndarray]]
    ) -> Optional[numpy.ndarray]:
        """
        Generate a ar gradient between two colors.

        Args:
            inputs (dict[str, numpy.ndarray | None]): Unused; generators
                produce output from parameters only.
        Returns:
            numpy.ndarray | None: A float32 gradient modifier of shape
                (width, height, 3) with values in [0, 1].
        """
        color_a = self.get_field_value("color_a")
        color_b = self.get_field_value("color_b")
        angle = self.get_field_value("angle")

        r_a, g_a, b_a, a_a = color_a
        r_b, g_b, b_b, a_b = color_b

        width, height = api.get_texture_resolution()
        y_idx, x_idx = numpy.indices((height, width), dtype=numpy.float32)

        radians = numpy.deg2rad(angle)
        direction_x = numpy.cos(radians)
        direction_y = numpy.sin(radians)

        cx, cy = width / 2.0, height / 2.0
        projection = (x_idx - cx) * direction_x + (y_idx - cy) * direction_y

        max_extent = numpy.sqrt(cx**2 + cy**2)
        t = (projection / max_extent + 1.0) / 2.0
        t = numpy.clip(t, 0.0, 1.0)

        color_a = numpy.array([b_a, g_a, r_a, a_a], dtype=numpy.float32) / 255.0
        color_b = numpy.array([b_b, g_b, r_b, a_b], dtype=numpy.float32) / 255.0

        result = (
            color_a[None, None, :] * (1 - t[:, :, None])
            + color_b[None, None, :] * t[:, :, None]
        )

        return result.astype(numpy.float32)
