...
class MoldNode(GeneratorNode):
    ...
    def process(
        ...
        num_drops = self.get_field_value("num_drops")
        drop_scale = self.get_field_value("drop_scale")
        grunge = self.get_field_value("grunge")
        ...
        for r_lo, r_hi, count_frac, strength, blur_frac in [
            ...
            for _ in range(n):
                ...
                r = rng.uniform(r_lo, r_hi) * height * drop_scale
                if r < 1.5:
                    continue
                cys.append(cy)
                cxs.append(cx)
        ...
        if speck.max() > 0.0:
            speck /= speck.max()
        speck = numpy.power(speck, 2.2 - grunge * 1.2)
        result -= speck * grunge * 0.15
